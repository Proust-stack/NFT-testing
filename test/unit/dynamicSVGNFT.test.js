...
!developmentChains.includes(network.name)
    ...
    : describe("Dynamic SVG NFT Unit Tests", function () {
          ...
          describe("mintNft", () => {
              it("emits an event and creats the NFT", async () => {
              ...
              it("Shifts the token URI to lower if the price does NOT surpass the highValue", async () => {
                  ...
                  const trx = await dynamicSvgNft.mintNFT(highValue)
                  await trx.wait(1)
                  const tokenURI = await dynamicSvgNft.tokenURI(0)
